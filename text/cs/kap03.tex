...
\section{Konceptuální model}\label{section:conceptual-model}
...
Při deserializaci objektu z \acrshort{json} v JavaScriptu, resp. TypeScriptu, dostaneme plain objekt.
...
Problém s referencemi lze vyřešit tím, že každá třída v modelu bude mít datovou složku \texttt{id}.
...
Obsahuje mnoho dalších metod kromě těch, které lze vidět na Obrázku~\ref{fig:utils-class-diagram}, například pro každý příkaz lze pozice (location) specifikovat i relativně (místo absolutně) k předchozí pozici.
