...
\begin{itemize}
  ...
  \item nomnoml~\cite{nomnoml22} k tvorbě UML diagramů~\cite{uml2017} psaním textu,
  \item Visual Paradigm Online~\cite{vpo22} pro tvorbu libovolných diagramů.
\end{itemize}
...
(který však nepřijímá změny od externích vývojářů~\cite{drawio22}) % https://github.com/jgraph/drawio/blob/c7122cad617f52563c11d90890e64ab06db3a27a/README.md#open-source-not-open-contribution
...
Bohužel, soubor se nepřenačítá automaticky, ale musí být manuálně synchronizován tlačítkem \menu{File > Synchronize (Alt+Shift+S)}, které je dostupné pouze v~desktopové verzi aplikace.
...
Diagrams.net sám o~sobě správu verzí neobsahuje, jen obvyklé \uv{Undo, Redo} pro aktuálního uživatele.
...
Menu \menu{Arrange > Layout} umožňuje celý diagram přearanžovat do zvoleného rozložení (Horizontal Flow, Vertical Flow, Horizontal Tree, Vertical Tree, Radial Tree, Organic, Circle, Org Chart, Parallels).
...
V menu \menu{Extras > Mathmatical Typesetting} lze povolit vykreslování matematické notace pomocí knihovny MathJax~\cite{mathjax2022}.
...
\begin{itemize}
  ...
  \item komercializace -- verze zdarma, měsíčně/ročně placené plány několika úrovní
\end{itemize}
...
Nástroj je svým rozhraním, designem a chováním nápadně podobný produktu diagrams.net ze sekce~\ref{section:digramsnet}.
Stejně tak spojování tvarů čarami má velmi podobné chování.
Jeden rozdíl je ten, že kresba spojení z jednoho tvaru do toho samého (tvorba \uv{cyklu}) je velmi nepředvídatelná.
Čára se při tom samovolně přemisťuje do všech možných stran, až nakonec zůstane přilepena na jedné ze stran tvaru.

Diagramy lze uložit do formátu \texttt{vpd}, který při otevření připomíná base64 kód.
Vypadá velice podobně jako serializace pro diagrams.net ze sekce~\ref{section:digramsnet}.
Nejedná se však o validní base64.

Jeho dekódováním nicméně vznikne nečitelný bajtový kód, který je buď zašifrovaný nebo v nezdokumentovaném proprietárním formátu.
