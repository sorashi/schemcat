\chapter{Existující nástroje}\label{chapter:existujici-nastroje}
...
Menu \menu{Arrange > Layout} umožňuje celý diagram uspořádat do zvoleného rozložení (Horizontal Flow, Vertical Flow, Horizontal Tree, Vertical Tree, Radial Tree, Organic, Circle, Org Chart, Parallels).
...
V menu \menu{Extras > Mathmatical Typesetting} lze povolit vykreslování matematické notace pomocí knihovny MathJax~\cite{mathjaxconsortium_mathjax_}.
...
Pomocí tlačítka \menu{Connect} lze poté přidat cizí klíč, který odkazuje do jiné tabulky tažením myši.
ERDPlus do tabulky přidá všechny primární klíče, které cílová tabulka obsahuje, jako nové sloupce.
K vytvoření cizího klíče, který odkazuje na stejnou tabulku (tzv. rekurzivní klíč) slouží tlačítko \menu{Recursive Key} ve vlastnostech tabulky.
...
Jak už bylo zmíněno, soubor \texttt{vpd} nemá úplně stejnou strukturu jako mxfile\footnote{\url{https://drawio-app.com/extracting-the-xml-from-mxfiles/}}.
...
\begin{table}[htb]
  ...
  \begin{center}
    ...
    \footnotesize
    \makebox[\textwidth]{
      \begin{tabular}{p{0.7in}lp{0.7in}p{0.7in}p{0.8in}p{0.8in}p{0.9in}}\toprule
        název                  & kategorie    & úložiště                            & interaktivní spolupráce                       & formát \mbox{projektu}                         & export                                           & komercializace                                                             \\ \midrule
        draw.io                & libovolné    & lokální, externí, prohlížeč         & částečně\tnote{\ref{tab:gc:external-storage}} & mxfile                                  & serializovaný, rastrový, vektorový, zjednodušený & zdarma\tnote{\ref{tab:gc:price-external-storage},\ref{tab:gc:open-source}} \\
        drawSQL                & logická      & online                              & placená                                       & JSON\tnote{\ref{tab:gc:plan}}           & schematický, rastrový                            & zdarma, předplacené plány\tnote{\ref{tab:gc:price-drawsql}}                \\
        ERDPlus                & konceptuální & online                              & ne                                            & JSON                                    & rastrový                                         & zdarma                                                                     \\
        nomnoml                & omezené      & ne\tnote{\ref{tab:gc:in-url}}       & ne                                            & nomnoml\tnote{\ref{tab:gc:custom-lang}} & rastrový, vektorový                              & zdarma\tnote{\ref{tab:gc:open-source}}                                     \\
  ...
  \begin{center}
    ...
    \makebox[\textwidth]{
      \begin{tabular}{lcccccccc}\toprule
        název                  & PNG & JPEG & SVG & PDF & HTML & SQL                                 & XML & JSON                                 \\ \midrule
        draw.io                & ano & ano  & ano & ano & ano  & ne                                  & ano & ne                                   \\
        drawSQL                & ano & ne   & ne  & ne  & ne   & ano                                 & ne  & ne\tnote{\ref{tab:fc:plan}}          \\
        ERDPlus                & ano & ne   & ne  & ne  & ne   & ano\tnote{\ref{tab:fc:erdplus-sql}} & ne  & ano\tnote{\ref{tab:fc:erdplus-json}} \\
        nomnoml                & ano & ne   & ano & ne  & ne   & ne                                  & ne  & ne                                   \\
        Visual Paradigm Online & ano & ano  & ano & ano & ne   & ne                                  & ne  & ne                                   \\ \bottomrule
      \end{tabular}
    }
  \end{center}
  ...
  \begin{center}
    ...
    \makebox[\textwidth]{
      \begin{tabular}{lccccccc}
        \toprule
        název                             & online & \makecell{paměť                                                                                                                                                                                     \\prohlížeče} & \makecell{Google \\Drive} & OneDrive & GitHub & GitLab & Dropbox \\ \midrule
        draw.io                           & ne     & ano             & ano                               & ano                               & ano                               & ano                               & ano                               \\
        drawSQL                           & ano    & ne              & ne                                & ne                                & ne                                & ne                                & ne                                \\
        ERDPlus                           & ano    & ne              & ne                                & ne                                & ne                                & ne                                & ne                                \\
        nomnoml                           & ne     & ano             & ne\tnote{\ref{tab:sc:workaround}} & ne\tnote{\ref{tab:sc:workaround}} & ne\tnote{\ref{tab:sc:workaround}} & ne\tnote{\ref{tab:sc:workaround}} & ne\tnote{\ref{tab:sc:workaround}} \\
        \makecell{Visual Paradigm Online} & ano    & ano             & ano                               & ne                                & ne                                & ne                                & ne                                \\  \bottomrule
      \end{tabular}    }
  \end{center}

  \footnotesize
  \begin{enumerate}[a.,ref = \alph*,noitemsep]
    \item nomnoml kromě webové aplikace poskytuje i lokální program pro převod diagramů z nomnoml jazyka, takže lze nomnoml soubor uložit do těchto úložišť a spravovat jeho verze\label{tab:sc:workaround}
  \end{enumerate}

  \caption{Úložiště existujících řešení}
