...
\subsection{Motivace}\mcomment{Nemám dát tuhle motivaci spíše do úvodu? Asi bych se měl trochu více rozepsat, ale zatím takhle.}
...
\begin{figure}[!htb]
  ...
  \mcomment{Je na obrázku korektní schematická kategorie?}
...
Díky tomu lze říci, že \acrshort{vsk} a schematická kategorie jsou na sebe vzájemně převoditelné a tedy jsou \hlcomment{konceptuálně ekvivalentní}{Spíše asi logicky ekvivalentní? Ale možná by se pletlo s \enquote{ekvivalentní v logické vrstvě}}.
...
\begin{algorithm}[htb]
  ...
  \SetKw{Return}{vrať}%
  \mcomment[blue]{Nemám celý algoritmus popsat spíš slovy? Ze začátku jsem to zkoušel, ale je tam tolik zanořování, že to začne být rychle nepřehledné. Případně tohle můžu dát do přílohy, protože je to přes 3 stránky.}%
  ...
  \For{složený atribut $C$ z \acrshort{er} diagramu}{
    ...
    \For{atribut $A$, který náleží $C$}{
      ...
      $M_A\gets$ vytvořBázovýMorfismus(identita $O_C$, identita $O_A$, $\bot$, (\one{}..\one{}), \hlmcomment{(\one{}..\one{})}{neměla by být "zpětná" kardinalita složených atributů spíš \zero{}..\many{}?})\;
    }
...
Algoritmus \hlcomment{skončí}{Musím ukázat i korektnost?}, pokud dostane korektní \acrshort{er} diagram (takže nemá cykly a je konečný), protože v cyklu \emph{dokud} na řádce \ref{alg:dokud} se určitě v každé iteraci najde alespoň jeden řešitelný entitní typ, díky neexistenci cyklů.
