...
Mějme tedy validní \acrshort{er} diagram. Převedeme ho na schematickou kategorii.
...
\begin{algorithm}[htb]
  ...
  \Fn{vytvořObjekt(název)}{
    $i\gets$ nová unikátní identita např.~z~$\N$\;
  ...
  \Fn{vytvořBázovýMorfismus(dom, kod, název, kardinalitaTam, kardinalitaZpět)}{
    $i\gets$ nová unikátní identita např.~z~$\N$\;
    $j\gets$ nová unikátní identita např.~z~$\N$\;
    ...
    vlož $M, M^{-1}$ do morfismů $\mathcal S$\;
    \Return $M$\;
  }
  \For{složené atributy $C$ z \acrshort{er} diagramu}{
    ...
    \For{atribut $A$, který náleží $C$}{
      ...
      $M_A\gets$ vytvořBázovýMorfismus(identita $O_C$, identita $O_A$, $\bot$, (\one..\one), (\one..\one))\;
    }
    do množiny identifikátorů $O_C$ přidej množinu signatur právě výše vytvořených $M_A$\;
  }
  \For{entitní typ $E$ z \acrshort{er} diagramu}{
...
Pro každý entitní typ $E$ z~\acrshort{er} diagramu vytvoříme odpovídající objekt schematické kategorie s trojicí složek (identita, název, množina identifikátorů).
Identitu přidělíme unikátní ze zvolené abecedy.
Název nastavíme na jméno entity z~\acrshort{er}.

Pro každý vlastní atribut nebo složený atribut $A$ entitního typy $E$.
...
\begin{enumerate}
  ...
  \item nebo se může jednat o slabý entitní typ (nemá ani jeden interní identifikátor).
\end{enumerate}
Hierarchie a externí identifikátory popíšeme později.
