...
\begin{itemize}
  \item $f$ se skládá z $(\sig_1, \dom_1, \cod_1, \name_1, (\minn_1, \maxx_1), \dup_1, \ord_1)$,
  \item $g$ se skládá z $(\sig_2, \cod_1, \cod_2, \name_2, (\minn_2, \maxx_2), \dup_2, \ord_2)$,
\end{itemize}
...
V $\N$ je určitě dostatek symbolů pro celou výslednou schematickou kategorii.
Unikátní symboly z $\N$ budeme značit $n$.
Použití $n$ tedy znamená \enquote{zde použijme unikátní symbol z $\N$}.
...
Pro každý složený atribut $C$ z \acrshort{er} diagramu a pro každý jeho atribut $A$ vytvoříme dva duální bázové morfismy mezi odpovídajícími objekty $O_C$ a $O_A$.
\begin{align*}
...
\end{align*}\mcomment{nebo prohodit?}
